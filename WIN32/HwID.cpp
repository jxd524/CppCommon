...
BOOL CHwID::WinNTHDSerialNumAsScsiRead( char* dwSerial, UINT* puSerialLen, UINT uMaxSerialLen )
{
    ...
    for( int iController = 0; iController < 4; ++ iController )
    {
		...
		CString driver_name;
		driver_name.Format(_T("\\\\.\\Scsi%d:"), iController);

        //  Windows NT, Windows 2000, any rights should do
        hScsiDriveIOCTL = CreateFile( driver_name,
            0, 
...
BOOL 
CHwID::DoIdentify( HANDLE hPhysicalDriveIOCTL, 
...
BOOL CHwID::WinNTHDSerialNumAsPhysicalRead( char* dwSerial, UINT* puSerialLen, UINT uMaxSerialLen )
{
#define  DFP_GET_VERSION          0x00074080
    BOOL bInfoLoaded = FALSE;

    for( UINT uDrive = 0; uDrive < 2; ++ uDrive )
    {
        ...
        //  and exit if can't.
		CString driver_name;
		driver_name.Format(_T("\\\\.\\PhysicalDrive%d"), uDrive);

        //  Windows NT, Windows 2000, must have admin rights
        hPhysicalDriveIOCTL = CreateFile( driver_name,
...
BOOL CHwID::GetMAC()
{
    ...
    if( dwResult == ERROR_BUFFER_OVERFLOW )
    {
		...
		IP_ADAPTER_INFO* p = new IP_ADAPTER_INFO();
        if( piai != NULL )
        {
            ...
            if( ERROR_SUCCESS == dwResult )
            {
                ...
                while( piai2 != NULL && ( m_str_len + piai2->AddressLength ) < 4096U )
                {
                    CopyMemory( m_buff + m_str_len, piai2->Address, piai2->AddressLength );
                    m_str_len += piai2->AddressLength;
                    piai2 = piai2->Next;                        
                }
            }
            else
...
BOOL CHwID::GetHdSn()
{
    ...
    {
        return WinNTHDSerialNumAsPhysicalRead( m_buff, &m_str_len, 1024 );
    }
}
...
BOOL CHwID::GetCpuId()
{
    ...
    if( !bException )
    {
        CopyMemory( m_buff + m_str_len, &uCpuID, sizeof( UINT ) );
        m_str_len += sizeof( UINT );

        uCpuID = strlen( ( char* )szCpu );
        CopyMemory( m_buff + m_str_len, szCpu, uCpuID );
        m_str_len += uCpuID;
...
bool CHwID::GetHwID( CStringA &AHash )
{
    m_str_len = 0;
    memset(m_buff, 0x0, 4096);
    ...
    MD5Buffer( m_buff, m_str_len, md5Buf );
    AHash = MD5ToStr( md5Buf ).c_str();
	return true;
}

//bool CHwID::GetHwID( MD5VAL* MD5Val)
//{
//	m_str_len = 0;
//	memset(m_buff, 0x0, 4096);
//
//	//CString str;
//	GetMAC();
//	//str = ConvertBufftoString(m_buff, m_str_len);
//	//NLOG(_T("MAC is %s, Lenth is %d"), str, m_str_len);
//
//	GetHdSn();
//	//str = ConvertBufftoString(m_buff, m_str_len);
//	//NLOG(_T("Hardisk is %s, Lenth is %d"),  str, m_str_len);
//
//	GetCpuId();
//	//str = ConvertBufftoString(m_buff, m_str_len);
//	//NLOG(_T("cpuid is %s, Lenth is %d"),  str, m_str_len);
//
//	if (m_str_len == 0 )
//	{
//		return false;
//	}
//	md5(m_buff, m_str_len, MD5Val);
//
//	return true;
//}
...
bool _cdecl CPUInfo::RetrieveClassicalCPUIdentity ()
{
	...
	switch (ChipManufacturer) {
		...
		case 3: STORE_CLASSICAL_NAME ("Pentium?II (0.28 祄)"); break;
		case 5: STORE_CLASSICAL_NAME ("Pentium?II (0.25 祄)"); break;
		case 6: STORE_CLASSICAL_NAME ("Pentium?II With On-Die L2 Cache"); break;
		case 7: STORE_CLASSICAL_NAME ("Pentium?III (0.25 祄)"); break;
		case 8: STORE_CLASSICAL_NAME ("Pentium?III (0.18 祄) With 256 KB On-Die L2 Cache "); break;
		case 0xa: STORE_CLASSICAL_NAME ("Pentium?III (0.18 祄) With 1 Or 2 MB On-Die L2 Cache "); break;
		case 0xb: STORE_CLASSICAL_NAME ("Pentium?III (0.13 祄) With 256 Or 512 KB On-Die L2 Cache "); break;
		...
		case 0:
			switch (ChipID.Model) {
		case 0: STORE_CLASSICAL_NAME ("Pentium?IV (0.18 祄)"); break;
		case 1: STORE_CLASSICAL_NAME ("Pentium?IV (0.18 祄)"); break;
		case 2: STORE_CLASSICAL_NAME ("Pentium?IV (0.13 祄)"); break;
		...
		case 6: STORE_CLASSICAL_NAME ("K6 (0.30 祄)"); break;
		case 7: STORE_CLASSICAL_NAME ("K6 (0.25 祄)"); break;
		case 8: STORE_CLASSICAL_NAME ("K6-2"); break;
		case 9: STORE_CLASSICAL_NAME ("K6-III"); break;
		case 0xd: STORE_CLASSICAL_NAME ("K6-2+ or K6-III+ (0.18 祄)"); break;
		default: STORE_CLASSICAL_NAME ("Unknown 80586 family"); return false;
			}
			break;
		case 6:
			switch (ChipID.Model) {
		case 1: STORE_CLASSICAL_NAME ("Athlon?(0.25 祄)"); break;
		case 2: STORE_CLASSICAL_NAME ("Athlon?(0.18 祄)"); break;
		...
		case 5:
			switch (ChipID.Model) {
		case 0: STORE_CLASSICAL_NAME ("mP6 (0.25 祄)"); break;
		case 2: STORE_CLASSICAL_NAME ("mP6 (0.18 祄)"); break;
